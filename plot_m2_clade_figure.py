...
#pylab.figure(1,figsize=(4.72441,2.5))
...
majority_axis.set_title('%s major clade' % parse_file.get_pretty_name(focal_population),loc='right',fontsize=5,y=0.89)
...
majority_axis.get_yaxis().set_tick_params(direction='out',length=2,pad=0)
majority_axis.get_xaxis().set_tick_params(direction='out',length=2,pad=0)
...
minority_axis.set_title('%s minor clade' % parse_file.get_pretty_name(focal_population),loc='right',fontsize=5,y=0.89)
...
minority_axis.get_yaxis().set_tick_params(direction='out',length=2,pad=0)
minority_axis.get_xaxis().set_tick_params(direction='out',length=2,pad=0)
...
extinct_axis.set_title('%s extinct' % parse_file.get_pretty_name(focal_population),loc='right',fontsize=5,y=0.89)
...
extinct_axis.get_yaxis().set_tick_params(direction='out',length=2,pad=0)
extinct_axis.get_xaxis().set_tick_params(direction='out',length=2,pad=0)
...
for i in xrange(0, len(remaining_populations)):
    ...
    remaining_axis.set_ylabel(figure_utils.get_short_pretty_name(remaining_populations[i]),fontsize=5,labelpad=-2,verticalalignment='center')
...
for mutation_idx in range(0,len(mutations))[starting_idx:]:
    ...
    if clade=='ancestral':
        ...
        if not remove_data_for_copyediting:
            fixed_axis.plot(masked_times, masked_freqs, '-', color='0.7', alpha=0.5, linewidth=0.25,zorder=1) #,rasterized=True)
    ...
    if clade=='majority':
        ...
        if not remove_data_for_copyediting:
            majority_axis.plot(masked_times, masked_freqs, '-', color='0.7', alpha=0.5, linewidth=0.25,zorder=1) #,rasterized=True)
    ...
    if clade=='minority':
        ...
        if not remove_data_for_copyediting:  
            minority_axis.plot(masked_times, masked_freqs, '-', color='0.7', alpha=0.5, linewidth=0.25,zorder=1) #,rasterized=True)
    ...
    if clade=='extinct':
        ...
        if not remove_data_for_copyediting:  
            extinct_axis.plot(masked_times, masked_freqs, '-', color='0.7', alpha=0.5, linewidth=0.25,zorder=1) #,rasterized=True)

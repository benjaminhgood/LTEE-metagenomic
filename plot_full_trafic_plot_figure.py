...
pylab.figure(1,figsize=(7.2,2.7))
fig = pylab.gcf()
...
for i in xrange(0,len(focal_populations)):
    ...
    axis.set_title('%s' % parse_file.get_pretty_name(focal_populations[i]),loc='right',fontsize=4,y=0.91)
...
for i in xrange(0,len(focal_populations)):
    ...
    for mutation_idx in xrange(0,len(mutations)):
        ...
        focal_axes[i].plot(masked_times, masked_freqs, '-o', alpha=0.5, markersize=1, markeredgecolor='none', zorder=4, linewidth=0.5)
...
fig.savefig(parse_file.figure_directory+'fig1.png',bbox_inches='tight', dpi=300, transparent=True)
pylab.close(fig)
sys.stderr.write("Done!\n")
sys.exit(1)
